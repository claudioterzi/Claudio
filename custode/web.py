...
<h2>Nuova scheda (compilarla quando si nasconde il tag)</h2>
<form class="scheda" method="post" action="{{ url_for('nuova_scheda') }}">
 ...
 <select name="categoria">
  <option value="libro">libro</option><option value="elettronica">elettronica</option>
  <option value="biancheria">biancheria</option><option value="arredo">arredo</option>
  <option value="cucina">cucina</option><option value="oggetto">altro</option>
 </select>
 ...
 <textarea class="largo" name="note" rows="2" placeholder="Note"></textarea>
...
@app.route("/")
def home():
    return render_template_string(PAGINA, schede=catalogo.tutte(), esito=None)
