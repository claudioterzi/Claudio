...
<div class="container">
  <header>
    ...
    <p class="versione">✦ Raffaello AI · v10 — il bottone apre il profumo 🧠 ✦</p>
  </header>
...
// Il bottone NON fa piu' chiamate in background (venivano bloccate su
// alcuni dispositivi/reti). Porta alla pagina composta DAL SERVER, che
// funziona in ogni browser perche' e' un semplice caricamento di pagina.
function chiediARaffaello(rilancio) {
  const i = leggi();
  if (!i.intenzione || i.intenzione === "senza intenzione") {
    document.getElementById("voce").textContent =
      "\\u2192 Scrivi prima la tua intenzione qui sopra.";
    return;
  }
  document.getElementById("voce").textContent =
    "\\ud83e\\udde0 Raffaello sta componendo\\u2026 (si apre tra pochi secondi)";
  let url = "/profumo?q=" + encodeURIComponent(i.intenzione) + "&ondata=" + i.ondata;
  if (i.fam) url += "&famiglia=" + encodeURIComponent(i.fam);
