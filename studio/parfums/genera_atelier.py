...
<div class="container">
  <header>
    ...
    <p class="versione">✦ Raffaello AI · v6 — ora risponde in ~5 secondi 🧠 ✦</p>
  </header>
  ...
  try {
    ...
    if (data.ok && data.parfum) {
      ...
      document.getElementById("voce").innerHTML =
        "\\u26a0 Raffaello non \\u00e8 riuscito a comporre in tempo (" + (data.errore || "errore") +
        "). \\u00c8 solo lentezza: <b>premi di nuovo il bottone grande</b>, di solito al secondo tentativo risponde.";
    }
  } catch (e) {
    document.getElementById("scheda").classList.remove("viva");
    document.getElementById("voce").innerHTML =
      "\\u26a0 Raffaello non ha risposto (rete lenta, o sei su GitHub Pages invece che su " +
      "claudio-ebon.vercel.app). <b>Riprova col bottone grande.</b>";
  } finally {
    btnC.disabled = false;
  }
}
