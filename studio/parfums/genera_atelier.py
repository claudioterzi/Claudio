...
<div class="container">
  ...
  <div class="banco">
    ...
    <div class="azioni">
      <button class="oro" id="componi">Componi</button>
...
function mostra(p) {
  corrente = p;
  document.getElementById("voce").textContent = VOCI[p.seme % VOCI.length];
  const righe = p.righe.map(r =>
    '<tr><td class="lv">' + r.livello + (r.livello === "scia" ? " \\u00b7 " + r.ruolo : "") + '</td>' +
  ...
  document.getElementById("scheda").innerHTML =
    ...
    '<div class="meta">' + p.stagione + " \\u00b7 " + p.momento +
    '<br><span class="badge">' + p.fattibilita + '</span></div></div>' +
    '<div class="col-tx">' +
    '<div class="sotto">Proposta dell\\u2019Atelier \\u00b7 ' + p.fam + " \\u00b7 " + p.stile.lezione + '</div>' +
    '<h2>' + p.nome + '</h2>' +
  ...
  document.getElementById("seme").textContent =
...
document.getElementById("scarica").addEventListener("click", () => {
  ...
  const testo = [
    ...
    "Stile: " + p.stile.lezione,
    "Seme: " + p.seme + " \\u00b7 variazione " + variazione,
    "",
    "CONCEPT", p.concept, "",
