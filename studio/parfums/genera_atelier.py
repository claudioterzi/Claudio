...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  header .versione { margin-top: 0.5rem; font-size: 0.68rem; letter-spacing: 0.14em;
...
function mostra(p) {
  ...
  document.getElementById("ancora").disabled = false;
...
let provateAI = [], intenzioneAI = "";

async function chiediARaffaello(rilancio) {
  const i = leggi();
  if (!rilancio || i.intenzione !== intenzioneAI) {
    provateAI = []; intenzioneAI = i.intenzione;
  }
  const btnC = document.getElementById("chiedi");
  const btnA = document.getElementById("ancora");
  btnC.disabled = true; btnA.disabled = true;
  document.getElementById("voce").textContent = provateAI.length
    ? "\\ud83e\\udde0 Raffaello cerca un\\u2019altra strada\\u2026 (prova " + (provateAI.length + 1) + ")"
    : "\\ud83e\\udde0 Raffaello annusa l\\u2019intenzione\\u2026 (pu\\u00f2 volerci qualche secondo)";
  document.getElementById("scheda").classList.remove("viva");
  const attesa = document.getElementById("voce");
  let ultimoErrore = "";
  try {
    // riprova automatico: fino a 3 tentativi. Il primo può fallire per
    // rete mobile o funzione fredda; il secondo di solito riesce.
    for (let tent = 1; tent <= 3; tent++) {
      if (tent > 1) {
        attesa.textContent = "\\ud83e\\udde0 Raffaello si sta svegliando\\u2026 (tentativo " + tent + " di 3)";
      ...
      try {
        ...
        if (data.ok && data.parfum) {
          provateAI.push(data.parfum.nome);
          mostra(daAI(data.parfum, i.intenzione));
          return;
        }
        ultimoErrore = data.errore || "errore";
      } catch (e) {
        ultimoErrore = "rete";
      }
    }
    document.getElementById("scheda").classList.remove("viva");
    attesa.innerHTML = "\\u26a0 Raffaello non \\u00e8 riuscito dopo 3 tentativi (" + ultimoErrore +
      "). Riprova tra un minuto col bottone grande \\u2014 se insiste, \\u00e8 la rete del telefono.";
  } finally {
    btnC.disabled = false;
  }
}
...
document.getElementById("ancora").addEventListener("click", () => {
  if (corrente && corrente.viaAI) { chiediARaffaello(true); return; }
  variazione += 1;
  const i = leggi();
  mostra(componi(i.intenzione, i.fam, i.stile, i.ondata, variazione));
});
