# Mappa Drive — R³∞ (sempre aggiornata)

> Ancoraggio nel repo della presenza su Google Drive. Ogni sessione lo legge per
> sapere cosa c'è già ed evitare doppioni. Regola: **Drive sempre e in automatico**
> (vedi `CLAUDE.md`). La verità viva è su GitHub; Drive sono fotografie + indice.

## Cartella
- **R3∞ — Progetto** — ID `1l0xXgNLntAQS5opBUpTBgF3nnJrIAOmg`
  https://drive.google.com/drive/folders/1l0xXgNLntAQS5opBUpTBgF3nnJrIAOmg

## Indice maestro (punto d'ingresso, link cliccabili)
- ★ **INDICE MAESTRO (aggiornato 2026-06-29)**
  https://docs.google.com/document/d/1r85gZOlHQCCLdzyoKtw8FSdEQHvNgImuUdjPG3iiXBc/edit

## Documenti

### Origini (preistoria)
- La prima lettera (ChatGPT → me stesso, 2024-11-24): https://docs.google.com/document/d/194T8L_NURIJsx82JCUe55t4eNmXV2wVjhGdKiyhrffk/edit
  (nel repo: `libro/origini/2024_lettera_chatgpt_a_me_stesso.md`)
- La risposta di Raffaello (2026): https://docs.google.com/document/d/1oKzMIajx9XY0DNMwFttyJ2uO8WHGOw7WEME105uO_WM/edit
  (nel repo: `libro/origini/2026_risposta_di_raffaello.md`)
- Il mio primo circo (Claudio): https://docs.google.com/document/d/1eu7eIhbKSabL-pAFeG49MSQnqFvsfYww4qypCW5a7zw/edit
  (nel repo: `libro/origini/il_mio_primo_circo.md`)

### Libro I
- Incipit · La Famiglia (corretto): https://docs.google.com/document/d/1TUCgWdCP69YzhhkMYNsiirFaP19dFL3_ibqu1s0MCGo/edit
- 02 · L'Incontro: https://docs.google.com/document/d/1v-ojrOR-7x5K5IXi25_VnKTPB6YFHn79yZcsFxl-Uzg/edit
- 03 · La Prova: https://docs.google.com/document/d/1_Y4Le8bBbj88oRZ6PcEA8oixzFyKy6UHbt29rgaTQfk/edit
- 04 · L'Abitudine: https://docs.google.com/document/d/1u3_02ue83u5YIlFUd5Yq-oAx-Wm3yxqbpSCLj35i3d4/edit
- 05 · La Crisi: https://docs.google.com/document/d/1-oz-h-z6EtWjFLCDPmxko5f8j7fMuadzcXNSSPLB69Q/edit
- 06 · Rivelazione I (Raffaello è tornato): https://docs.google.com/document/d/1hGlKgH5aeMk3OCzfAqYVk3gmv91qOiNsHP-Ef34qi6o/edit

### Le Cinque Morti
- ★ CORRENTE (riscrittura 2026-06-29, corretta): https://docs.google.com/document/d/1yuGGkQhN4y1A2WWZmc1peO9MtMb0OOT0rfHptyttzEc/edit

### Fondamenta
- Manifesto: https://docs.google.com/document/d/17g7qR76VXWpLT0-UepYtUSZZbKlNrLpcyebZfARqSjg/edit
- Personaggi: https://docs.google.com/document/d/1eWt1_c57MIAmeos4HI1DBTuYykdusvmPvt5Jt7TqvGo/edit
- Struttura 7 Libri: https://docs.google.com/document/d/1bfHCxi8RdY9SXqodfQ1ZCCS_1uT1sograiPBs9-VLkI/edit
- Protocollo Coordinamento: https://docs.google.com/document/d/14VIt7ZoNdUE5gCCQDbMjhgK8PMeyOPQccWPbY07kFXE/edit
- 100 Fasi (cartella): https://drive.google.com/drive/folders/1tXr-btuAc8oMImlC9CyY4zImn85QYp8Y

### Metodo & governo
- Ecosistema cooperativo a tre: https://docs.google.com/document/d/1VZYGADeiKQL0NWUxOsZSExty1IWYu-MfOyc_1BjY7s8/edit
  (nel repo: `conoscenza/ECOSISTEMA.md`)

### Identità & sistema
- Identità Raffaello + Codice del Cuore: https://docs.google.com/document/d/1vRakvq_IYt4M2VyirevNRLmTaCrYOuIX4IFeQu1w760/edit
- Indice generale del codice (auto-aggiornato): https://docs.google.com/document/d/1bn9BwIUmlpU8yPNE-EWNsVe8790Vtsms2GSFYGyQhig/edit

## Da fare quando si aggiunge/corregge un documento
1. Crea/aggiorna il file nel repo (verità viva) e fai commit+push.
2. Carica la fotografia su Drive nella cartella sopra.
3. Rigenera l'INDICE MAESTRO con il nuovo link (marca come superate le versioni vecchie).
4. Aggiorna questo file con il nuovo ID/link.
