(function(){
  ...
  function relationLabel(t){return({family:'famiglia',contains:'appartenenza',genealogy:'genealogia',canon:'canone',version:'versione',symbolic:'legame simbolico',method:'metodo',derives:'derivazione',echo:'eco',source:'fonte',theme:'tema',style:'stile',historical:'asse storico'})[t]||'relazione'}

  function visibleSet(){
    const q=normalize(searchEl.value.trim()),cat=categoryEl.value,year=yearEl.value?Number(yearEl.value):null;
    const works=workNodes.filter(n=>{if(cat&&n.category!==cat)return false;if(year&&n.year!==year)return false;if(q&&!(normalize(n.title).includes(q)||normalize(n.category).includes(q)))return false;return true});
  ...
  function renderCatalog(ids){
    ...
    categories.forEach(cat=>{const rows=visibleWorks.filter(n=>n.category===cat).sort((a,b)=>a.year-b.year||b.importance-a.importance||a.title.localeCompare(b.title,'it'));if(!rows.length)return;const sec=document.createElement('section');sec.className='catalog-group';const h=document.createElement('h3');h.textContent=cat+' · '+rows.length;sec.appendChild(h);const grid=document.createElement('div');grid.className='title-grid';rows.forEach(n=>{const a=document.createElement('a');a.className='title-link';a.style.setProperty('--node-color',n.color);a.href=n.url;a.target='_blank';a.rel='noopener';a.textContent=n.title;a.title='Apri lo scritto · '+n.year;a.addEventListener('mouseenter',()=>focusNode(n,false));grid.appendChild(a)});sec.appendChild(grid);catalogGroupsEl.appendChild(sec)})
  }
  ...
  function renderDetail(node){
    const related=relatedNodes(node);detailEl.innerHTML=`<p class="detail-kicker">${escapeHtml(node.kind==='work'?node.category:'Nodo di orientamento')}</p><h2>${escapeHtml(node.title)}</h2><p>${node.kind==='work'?'Questo titolo occupa una posizione determinata dal suo momento nella genealogia, dalla centralità nel corpus e dalla quantità di relazioni con gli altri testi.':'Nodo di orientamento: raccoglie una costellazione del corpus e permette di leggerne la struttura.'}</p><div class="meta"><div class="chip"><strong>${escapeHtml(node.year)}</strong><span>tempo</span></div><div class="chip"><strong>${escapeHtml(node.importance)}/10</strong><span>importanza</span></div><div class="chip"><strong>${escapeHtml(node.density)}/10</strong><span>densità</span></div></div>${node.kind==='work'?`<a class="open-doc" href="${escapeHtml(node.url)}" target="_blank" rel="noopener">Apri lo scritto ↗</a>`:''}<div class="related"><h3>Connessioni · ${related.length}</h3>${related.slice(0,18).map(r=>`<a href="#${escapeHtml(r.node.id)}" data-focus="${escapeHtml(r.node.id)}"><span>${escapeHtml(r.node.title)}</span> · <small>${escapeHtml(relationLabel(r.type))}</small></a>`).join('')}</div>`;
  ...
  function initGraph(){
    ...
    try{
      graph=ForceGraph3D({controlType:'orbit',rendererConfig:{antialias:true,alpha:true}})(graphEl).graphData({nodes:allNodes,links:allLinks}).backgroundColor('rgba(0,0,0,0)').showNavInfo(false).nodeThreeObject(makeNodeObject).nodeLabel(n=>`<div style="max-width:260px;padding:5px 7px"><b>${escapeHtml(n.title)}</b><br><span style="opacity:.75">${escapeHtml(n.category)} · ${n.year}</span></div>`).nodeVal(n=>n.kind==='hub'?11:n.kind==='category'?7:2.5+n.importance*.42).nodeColor(n=>n.color).linkColor(l=>l.type==='genealogy'?'#d7b65b':l.type==='version'?'#b88fd2':'#536073').linkOpacity(.38).linkWidth(l=>l.type==='genealogy'?1.25:.35).linkDirectionalParticles(l=>['genealogy','derives','source'].includes(l.type)?2:0).linkDirectionalParticleWidth(1.6).linkDirectionalParticleSpeed(.004).onNodeClick(n=>focusNode(n,true)).onNodeHover(n=>{graphEl.style.cursor=n?'pointer':'grab'}).cooldownTicks(0).warmupTicks(0);
