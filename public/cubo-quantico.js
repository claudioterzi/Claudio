(function(){
  ...
  function renderDetail(node){
    ...
    detailEl.innerHTML=`<p class="detail-kicker">${escapeHtml(node.kind==='work'?node.category:'Nodo di orientamento')}</p><h2>${escapeHtml(node.title)}</h2>${dateLine}<p>${escapeHtml(description)}</p><div class="meta"><div class="chip"><strong>${escapeHtml(node.year)}</strong><span>tempo</span></div><div class="chip"><strong>${escapeHtml(node.importance)}/10</strong><span>importanza</span></div><div class="chip"><strong>${escapeHtml(node.density)}/10</strong><span>densità</span></div></div>${node.kind==='work'?`<a class="open-doc" href="${escapeHtml(node.url)}" target="_blank" rel="noopener">Apri lo scritto ↗</a>`:''}<div class="related"><h3>Connessioni · ${related.length}</h3>${related.slice(0,18).map(r=>`<a href="#${escapeHtml(r.node.id)}" data-focus="${escapeHtml(r.node.id)}"><span>${escapeHtml(r.node.title)}</span> · <small>${escapeHtml(relationLabel(r.type))}</small></a>`).join('')}</div>`;
