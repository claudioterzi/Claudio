(function(){
  ...
  function linkEvidence(l){return String(l.confidence||l.evidence||'DECLARED').toUpperCase()}
  function evidenceLabel(v){return({DECLARED:'dichiarata',EXTRACTED:'estratta',INFERRED:'inferita',AMBIGUOUS:'ambigua',POSSIBLE:'possibile',HISTORICAL:'storica',REJECTED:'scartata'})[String(v||'').toUpperCase()]||String(v||'').toLowerCase()}
  function relationLabel(t){return({family:'famiglia',contains:'appartenenza',genealogy:'genealogia',canon:'canone',version:'versione',symbolic:'legame simbolico',method:'metodo',derives:'derivazione',echo:'eco',source:'fonte',theme:'tema',style:'stile',historical:'asse storico',threshold:'soglia fondativa',calls:'chiama',imports:'importa',implements:'implementa',references:'riferisce',cites:'cita',conceptually_related_to:'relazione concettuale',semantically_similar_to:'somiglianza semantica',shares_data_with:'scambio dati',rationale_for:'razionale di',related:'relazione'})[t]||String(t||'relazione').replaceAll('_',' ')}
  ...
  function relatedNodes(node){const rows=[];allLinks.forEach(l=>{const[s,t]=linkEnds(l),meta={type:l.type,evidence:linkEvidence(l),sourceSystem:l.sourceSystem||'',sourceFile:l.sourceFile||l.source_file||'',confidenceScore:l.confidenceScore??l.confidence_score};if(s===node.id&&nodeById.has(t))rows.push(Object.assign({node:nodeById.get(t)},meta));else if(t===node.id&&nodeById.has(s))rows.push(Object.assign({node:nodeById.get(s)},meta))});return rows.filter(r=>r.node.id!=='corpus-claudio-terzi').sort((a,b)=>b.node.importance-a.node.importance)}
