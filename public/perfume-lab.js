...
(function (root) {
  ...
  function mount(host, perfume) {
    ...
    const saved = perfume.lab_draft;
    if (saved) {
      basis.value = saved.basis; concentration.value = saved.concentration;
      density.value = saved.density; support.value = saved.support;
      preparations.forEach((input,i)=>{ input.value = saved.preparations.find(p=>p.n===recipe[i][1])?.preparation || ''; });
    }
    root.TerziLab.snapshot = () => ({basis:basis.value, concentration:concentration.value,
      density:density.value, support:support.value,
      preparations:recipe.map((r,i)=>({n:r[1],preparation:preparations[i].value.trim()})),
