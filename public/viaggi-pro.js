...
function hashId(s){let h=2166136261;for(let i=0;i<s.length;i++){h^=s.charCodeAt(i);h=Math.imul(h,16777619)}return (h>>>0).toString(16).padStart(8,'0')}
...
function compactCandidate(x,ctx,certainty,scope){const identity=fingerprint(ctx)+'|'+signature(x);return {
  id:'F-'+hashId(identity),source:'api-live',provider:x.provider_ricerca||x.provider||((x.voli||[])[0]?.vettore)||'rete-live',
...
function writeBest(snapshot){const list=bests(),fp=snapshot.fingerprint;const kept=list.filter(x=>x.fingerprint!==fp);kept.push(snapshot);save(BEST_KEY,kept.slice(-MAX_BEST))}
...
window.RaffaelloFlightEnrichment={ingest,bestFor,history,bests,recordManual,normalizePlace,version:'2.0.1'};
