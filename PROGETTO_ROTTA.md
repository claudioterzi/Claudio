# PROGETTO ROTTA — Viaggio e Operatività al Rientro

> La rotta è il viaggio + ciò che lo segue.
> Non finisce all'atterraggio: l'arrivo a Bruxelles apre i dossier operativi.
> Ultimo aggiornamento: 2026-06-22

---

## Tratta in corso — 23 giugno 2026

**Volo Ryanair · PNR H549QQ · BGY → CRL**
Bergamo Orio al Serio → Bruxelles South Charleroi
✅ Boarding pass: **fatto**

### Timeline del giorno

| Ora | Azione |
|---|---|
| 11:30 | Uscita di casa |
| ~12:30 | Arrivo aeroporto BGY — controlli sicurezza |
| 13:30 | **Gate close** (non arrivare dopo) |
| 14:00 | Decollo BGY |
| ~15:45 | Atterraggio CRL (stessa fascia oraria CEST) |
| ~16:15 | Uscita aeroporto, transfer verso Bruxelles |
| ~17:15 | Arrivo Bruxelles centro |

### Da Charleroi (CRL) a Bruxelles

| Opzione | Tempo | Note |
|---|---|---|
| **Flibco shuttle** | ~1h | Diretto CRL → Bruxelles-Midi, prenotabile online |
| TEC bus A + treno | ~1h15 | Bus per Charleroi-Sud, poi treno SNCB per Bruxelles |

**Consiglio:** prenotare Flibco in anticipo (più economico online che a bordo).

### Checklist pre-partenza

- [x] Boarding pass scaricato
- [ ] Documento d'identità / passaporto (YB6497683)
- [ ] Bagaglio entro limiti Ryanair (verificare dimensioni cabina)
- [ ] Transfer CRL→Bruxelles prenotato
- [ ] Power bank carico (telefono = boarding pass)

---

## Airbnb — Gestione ospiti al rientro

> Il rientro del 23 giugno non è libero: c'è l'appartamento da gestire come host.
> Prima di attaccare qualsiasi dossier operativo, queste sono le priorità fisiche.

### Checklist host al rientro

- [ ] Pulizie appartamento
- [ ] Consegna chiave agli ospiti (o coordinate di accesso)
- [ ] Messaggi di benvenuto / coordinamento ai clienti
- [ ] Verificare stato appartamento

### Dove dormi

Mentre gli ospiti occupano l'appartamento principale:
- **Stanzetta di servizio** — lavatrice disponibile, no bagno privato
- **Durata stimata:** max 2–3 giorni
- Appena gli ospiti fanno checkout → rientro nel tuo spazio normale

### Date critiche — calendario Airbnb

| Evento | Data | Ora | Note |
|---|---|---|---|
| Check-in ospiti attuali | *(da calendario)* | | |
| Check-out ospiti attuali | *(da calendario)* | | → da qui sei libero |
| Prossimo check-in | *(da calendario)* | | |

**Per collegare il calendario Airbnb:**
1. Vai su Airbnb → Annunci → il tuo annuncio → Disponibilità → Collegamento calendario
2. Copia il link iCal (formato `.ics`)
3. Su Google Calendar → Aggiungi calendario → Da URL → incolla il link
4. Il feed si aggiorna automaticamente e appare qui nel calendario

> Una volta collegato: tutte le date check-in/check-out sono visibili in Google Calendar
> e possiamo costruire la timeline operativa precisa attorno ad esse.

---

## All'arrivo a Bruxelles — la rotta operativa

Tornato a casa, questi sono i dossier pronti da attaccare. Tutta la
documentazione di supporto è già su Drive (vedi `DRIVE_LINKS.md`).

### Priorità 1 — ASBL SkyRights Foundation
- **Cosa:** registrare la fondazione (prerequisito di ogni grant EU)
- **Come:** procedura completa in ASBL-001 su Drive
- **Costo/tempo:** ~200€ · 3-5 settimane
- **Primo passo concreto:** scaricare template statuti CSA da prolegal.be,
  adattare oggetto sociale, aprire egreffe.be con eID/itsme®

### Priorità 2 — ARGO Heartbeat (attivazione)
- **Cosa:** attivare la pulsazione oraria del sistema
- **Come:** script.google.com → incolla `argo_heartbeat.gs` →
  Script Properties `GEMINI_API_KEY` → `installaTrigger()` → `testHeartbeat()`
- **Tempo:** 15 minuti

### Priorità 3 — Accessi satellite
- **Planet Labs E&R:** domanda come ricercatore indipendente (guida PLANET-003)
- **Maxar:** registrazione account developer (guida MAXAR-002)
- **Nel frattempo:** Sentinel-2 è gratuito e senza account

### Priorità 4 — Fondi EU
- **UNHCR Innovation Fund** (rolling, nessuna scadenza) — candidatura più rapida
- **Horizon Europe Cluster 3** — scadenza 15/09/2026, serve consorzio
- Dettagli in EU-FUNDING-004 su Drive

---

## Stato e prossimo passo

| Fase | Stato |
|---|---|
| Boarding pass | ✅ fatto |
| Viaggio | ⏳ 23 giugno |
| Transfer CRL→Bruxelles | ⬜ da prenotare |
| **Airbnb calendario collegato** | ⬜ iCal URL da aggiungere a Google Calendar |
| Pulizie + chiave + messaggi clienti | ⬜ al rientro (priorità 0) |
| Stanzetta: max 2-3 giorni | ⏳ finché ospiti non fanno checkout |
| Dossier operativi (ASBL, ARGO, ecc.) | 📋 pronti — partono dopo il checkout |

**Prossimo passo immediato:** prenotare il transfer Charleroi→Bruxelles.
**Prossimo passo sistema:** aggiungere il link iCal Airbnb a Google Calendar per sbloccare le date esatte.

---

*Progetto Rotta — parte del sistema SDQ-1 di Claudio Terzi, Bruxelles*
